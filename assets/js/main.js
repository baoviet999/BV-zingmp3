...
const app = {
    ...
    listenList:[
        ...
        },
        {
            ...
            content : 'Giai Điệu Và Trào Lưu Gây Sốt',
            singer1: 'Doja Cat',
            singer2: 'The Kid',
            singer3: 'LaRoi'
        },
    ...
    handleEvent: function () {
        ...
        console.log(themesBg);

        Array.from(themesBg).forEach(function (themeBg, index) {
            var headerColor = ["#e7dfdd", "blue", "green", "yellow", "orange"];
            var bgImage = [
            ...
            var navTextColor = ["#000", "#1df", "#1da", "#2ee", "#6fd"];
            var playColor = ["#d0ccc5", "#1df", "#1da", "#2e3", "#2a2"];
            var searchColor = ["#d0ccc5", "#1df", "#1da", ""];

            var navText = navTextColor[index];
            var bgTheme = bgImage[index];
            var colorHeader = headerColor[index];
            var playSongColor = playColor[index];
            var searchBgColor = searchColor[index];
            themeBg.onclick = function (e) {
                $(".content-header").style.backgroundColor = colorHeader;
                $(".app").style.backgroundImage = `url(${bgTheme})`;
                Array.from($$(".app a")).forEach(function (a, i) {
                    a.setAttribute("style", `color : ${navText} !important`);
                });
                $(".app-play").style.backgroundColor = playSongColor;
                $(".content-header__search-input").style.backgroundColor = searchBgColor;
            };
        });
    },
